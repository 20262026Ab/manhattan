...
@st.cache_data
def load_all():
    # ── GİDER VERİSİ (gelir.csv dosyasında!) ─────────────────────────────
    graw = pd.read_csv("gelir.csv", sep=";", header=None, engine="python", on_bad_lines="skip")
    ...
    df_ihtiyac = graw.iloc[2:16].copy()
    ...
    # ── GELİR VERİSİ (gider.csv dosyasında!) ────────────────────────────
    araw = pd.read_csv("gider.csv", sep=";", header=None, engine="python", on_bad_lines="skip")
...
try:
    ...
    with tab5:
        ...
        fig_bar.add_trace(go.Bar(name="Toplam Gelir", x=df_aylik["Ay"], y=df_aylik["Gelir"],
                                  ...
                                  textposition="outside"))
        fig_bar.add_trace(go.Bar(name="Toplam Gider", x=df_aylik["Ay"], y=df_aylik["Gider"],
                                  marker_color="#EF553B",
